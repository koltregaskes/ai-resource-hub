...
export function getJobCompanies(): DBJobCompany[] {
  const db = getDB();
  return db.prepare(`
    SELECT jc.*, p.name AS provider_name, p.colour AS provider_colour
...
export function getAIJobs(limit: number = 200): DBAIJob[] {
  ...
  return db.prepare(`
    SELECT
      ...
      p.colour AS provider_colour
    FROM ai_jobs j
    JOIN job_companies jc ON j.company_id = jc.id
    LEFT JOIN providers p ON j.provider_id = p.id
    WHERE j.active = 1
    ORDER BY
      CASE j.function_category
        ...
        ELSE 5
      END,
      COALESCE(j.posted_at, j.listed_at, j.updated_at) DESC,
      j.title ASC
    LIMIT ?
  `).all(limit) as DBAIJob[];
}

export function getAIJobFunctionCounts(): Array<{ function_category: string; count: number }> {
  const db = getDB();
  return db.prepare(`
    SELECT function_category, COUNT(*) AS count
    FROM ai_jobs
    WHERE active = 1
    GROUP BY function_category
    ORDER BY count DESC, function_category
  `).all() as Array<{ function_category: string; count: number }>;
}

export function getAIJobLocationCounts(limit: number = 12): Array<{ location_group: string; count: number }> {
  const db = getDB();
  return db.prepare(`
    SELECT COALESCE(location_group, 'Other') AS location_group, COUNT(*) AS count
    FROM ai_jobs
    WHERE active = 1
    GROUP BY COALESCE(location_group, 'Other')
    ORDER BY count DESC, location_group
    LIMIT ?
  `).all(limit) as Array<{ location_group: string; count: number }>;
}
...
export function getLatestAIJobSnapshots(): DBAIJobSnapshot[] {
  ...
  return db.prepare(`
    ...
    JOIN job_companies jc ON s.company_id = jc.id
    LEFT JOIN providers p ON s.provider_id = p.id
    WHERE s.snapshot_date = (SELECT MAX(snapshot_date) FROM ai_job_snapshots)
    ORDER BY s.open_role_count DESC, jc.name ASC
  `).all() as DBAIJobSnapshot[];
}
...
export function getAIJobSnapshotSeries(limitDays: number = 30): Array<{
  ...
  return db.prepare(`
    SELECT
      snapshot_date,
      SUM(open_role_count) AS open_role_count,
      SUM(remote_role_count) AS remote_role_count,
      SUM(research_role_count) AS research_role_count,
      SUM(engineering_role_count) AS engineering_role_count,
      SUM(product_role_count) AS product_role_count,
      SUM(gtm_role_count) AS gtm_role_count,
      SUM(operations_role_count) AS operations_role_count,
      SUM(other_role_count) AS other_role_count
    FROM ai_job_snapshots
    GROUP BY snapshot_date
    ORDER BY snapshot_date DESC
    LIMIT ?
  `).all(limitDays).reverse() as Array<{
    snapshot_date: string;
    open_role_count: number;
    remote_role_count: number;
    research_role_count: number;
...
export function getAIJobsOverview(): {
  ...
  const latestSnapshot = db.prepare(`
    SELECT
      ...
      SUM(open_role_count) AS total_open_roles,
      COUNT(*) AS tracked_companies,
      SUM(remote_role_count) AS remote_roles,
      SUM(research_role_count) AS research_roles,
      SUM(engineering_role_count) AS engineering_roles,
      SUM(product_role_count) AS product_roles,
      SUM(gtm_role_count) AS gtm_roles,
      SUM(operations_role_count) AS operations_roles,
      SUM(other_role_count) AS other_roles
    FROM ai_job_snapshots
    WHERE snapshot_date = (SELECT MAX(snapshot_date) FROM ai_job_snapshots)
  `).get() as {
    latest_snapshot_date: string | null;
    total_open_roles: number | null;
    tracked_companies: number | null;
    remote_roles: number | null;
    research_roles: number | null;
    engineering_roles: number | null;
    product_roles: number | null;
    gtm_roles: number | null;
    operations_roles: number | null;
    other_roles: number | null;
  } | undefined;

  const freshness = db.prepare(`
    SELECT
      COUNT(*) AS total_open_roles,
      SUM(CASE WHEN posted_at IS NOT NULL THEN 1 ELSE 0 END) AS dated_roles,
      SUM(CASE WHEN posted_at >= datetime('now', '-14 days') THEN 1 ELSE 0 END) AS posted_last_14d,
      SUM(CASE WHEN posted_at >= datetime('now', '-30 days') THEN 1 ELSE 0 END) AS posted_last_30d
    FROM ai_jobs
    WHERE active = 1
  `).get() as {
    total_open_roles: number | null;
    dated_roles: number | null;
    posted_last_14d: number | null;
    posted_last_30d: number | null;
  } | undefined;

  const totalOpenRoles = latestSnapshot?.total_open_roles ?? 0;
  const remoteRoles = latestSnapshot?.remote_roles ?? 0;

  return {
    total_open_roles: totalOpenRoles,
    tracked_companies: latestSnapshot?.tracked_companies ?? 0,
    remote_roles: remoteRoles,
    remote_share: totalOpenRoles > 0 ? (remoteRoles / totalOpenRoles) * 100 : 0,
    dated_roles: freshness?.dated_roles ?? 0,
    posted_last_14d: freshness?.posted_last_14d ?? 0,
    posted_last_30d: freshness?.posted_last_30d ?? 0,
    research_roles: latestSnapshot?.research_roles ?? 0,
    engineering_roles: latestSnapshot?.engineering_roles ?? 0,
    product_roles: latestSnapshot?.product_roles ?? 0,
    gtm_roles: latestSnapshot?.gtm_roles ?? 0,
    operations_roles: latestSnapshot?.operations_roles ?? 0,
    other_roles: latestSnapshot?.other_roles ?? 0,
    latest_snapshot_date: latestSnapshot?.latest_snapshot_date ?? null,
  };
}
